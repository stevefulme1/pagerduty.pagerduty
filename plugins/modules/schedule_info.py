...
schedules:
  ...
  contains:
    ...
    schedule:
      description: >-
        A single schedule resource returned by the PagerDuty API.
      type: dict
...
def fetch_list(client, module):
    ...
    if offset is not None or limit is not None:
        ...
        if limit is not None:
            params["limit"] = limit
        response = client.get("/schedules", params=params)
        if isinstance(response, dict):
            return response.get("schedules", response.get("data", response.get("items", [])))
        return response if isinstance(response, list) else []
    else:
        return client.get_paginated("/schedules", params=params)
