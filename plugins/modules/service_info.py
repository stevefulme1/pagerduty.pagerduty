...
---
module: service_info
short_description: Retrieve information about service resources
version_added: "0.1.0"
description:
  - Retrieve a single service by its identifier, or list all service resources.
...
options:
  id:
    description:
      ...
      - When omitted, all service resources are listed.
    type: str
    required: false
...
- name: List all service resources
  pagerduty.pagerduty.service_info:
  register: result
...
RETURN = r"""
services:
  description: List of service resources matching the query.
  returned: always
  type: list
  elements: dict
  contains:
    ...
    service:
      description: >-
        A single service resource returned by the PagerDuty API.
      type: dict
...
def fetch_single(client, identifier):
    """Retrieve a single service by identifier."""
...
def fetch_list(client, module):
    ...
    if offset is not None or limit is not None:
        ...
        if isinstance(response, dict):
            return response.get("services", response.get("data", response.get("items", [])))
...
def main():
    ...
    try:
        ...
        if identifier is not None:
            ...
            result["services"] = [item] if item else []
        else:
            result["services"] = fetch_list(client, module)
