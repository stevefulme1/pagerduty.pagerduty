...
DOCUMENTATION = r'''
---
module: event_orchestration_global_rule_info
short_description: List global rules for a PagerDuty event orchestration
description:
  - Retrieve information about PagerDuty rules.
...
options:
  orchestration_id:
    description: The ID of the event orchestration.
    type: str
    required: true
seealso:
  - module: pagerduty.pagerduty.event_orchestration_global_rule
notes:
...
EXAMPLES = r'''
- name: List all rules
  pagerduty.pagerduty.event_orchestration_global_rule_info:
    orchestration_id: EXAMPLE_ID
  register: result
'''

RETURN = r'''
rules:
  description: List of rules matching the query.
  type: list
  elements: dict
  contains:
    id:
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            orchestration_id=dict(type='str', required=True),
    ...
    try:
        qp = {}
        data = client.list_all('/event_orchestrations/' + params['orchestration_id'] + '/global/rules', 'rules', params=qp or None)
