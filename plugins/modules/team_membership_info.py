...
DOCUMENTATION = r'''
---
module: team_membership_info
short_description: List members of a PagerDuty team
description:
  - Retrieve information about PagerDuty members.
...
options:
  team_id:
    ...
    required: true
seealso:
  - module: pagerduty.pagerduty.team_membership
notes:
...
- name: List all members
  pagerduty.pagerduty.team_membership_info:
    team_id: EXAMPLE_ID
  register: result
'''
...
RETURN = r'''
members:
  description: List of members matching the query.
  type: list
  elements: dict
  contains:
    id:
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            team_id=dict(type='str', required=True),
    ...
    try:
        qp = {}
        data = client.list_all('/teams/' + params['team_id'] + '/members', 'members', params=qp or None)
        module.exit_json(changed=False, members=data, count=len(data))
