...
def _build_resource(**overrides):
    """Return a mock team resource dict."""
    base = {
        "id": "res-123",
        "team": "test-team"
    }
...
def resource_args(module_args):
    """Module args for team operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_matching_resource(self, resource_args):
        ...
        mock_client.get.return_value = {"team": existing}

        mock_module = MagicMock()
        mock_module.params = resource_args

        from ansible_collections.pagerduty.pagerduty.plugins.modules.team import get_current_state
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.pagerduty.pagerduty.plugins.modules.team import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.pagerduty.pagerduty.plugins.modules.team import build_payload
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(team="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(team="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
