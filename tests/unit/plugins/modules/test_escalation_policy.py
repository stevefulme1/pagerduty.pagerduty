...
def _build_resource(**overrides):
    """Return a mock escalation_policy resource dict."""
    base = {
        "id": "res-123",
        "escalation_policy": "test-escalation_policy"
    }
...
def resource_args(module_args):
    """Module args for escalation_policy operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_matching_resource(self, resource_args):
        ...
        mock_client.get.return_value = {"escalation_policy": existing}

        mock_module = MagicMock()
        mock_module.params = resource_args

        from ansible_collections.pagerduty.pagerduty.plugins.modules.escalation_policy import get_current_state
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.pagerduty.pagerduty.plugins.modules.escalation_policy import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.pagerduty.pagerduty.plugins.modules.escalation_policy import build_payload
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        resource_args["escalation_policy"] = "new-value"
        mock_module = MagicMock()
        mock_module.params = resource_args
        mock_module.check_mode = False
        mock_ansible_cls.return_value = mock_module
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(escalation_policy="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(escalation_policy="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
